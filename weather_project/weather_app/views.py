...
def index(request):
    api_key = '78a3072be08e766541b671e00917e38a'
...
def fetch_weather_and_forecast(city, api_key, current_weather_url, forecast_url):
    ...
    print(f"Fetching current weather data from URL: {current_weather_url_formatted}")
    response = requests.get(current_weather_url_formatted).json()

    if 'coord' not in response:
        print(f"Error: 'coord' key not found in the current weather response for city: {city}")
        print(f"Current weather response: {response}")
        return None, []

    lat, lon = response['coord']['lat'], response['coord']['lon']
    forecast_url_formatted = forecast_url.format(lat, lon, api_key)
    print(f"Fetching forecast data from URL: {forecast_url_formatted}")
